...
RPCEmu Extended is built for Linux, Windows and macOS from one source tree,
...
| `test_cpu_6800` | the 6800, 6802 and 6808 core: the quirks rather than the instruction set - CPX not touching the carry, INX affecting only Z, TST clearing it, V after a shift, the stack pointer's off-by-one, and the opcodes this part and the 6809 disagree about |
...
| `test_cpu_cycles` | 6502, Z80, 6809 and 6800 instruction timings against published values, both polarities of every conditional, every 6809 postbyte's own cost, and that a 6800's indexed form costs MORE than its extended one where a 6809's costs less |
