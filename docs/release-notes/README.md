...
- **Say which build an asset is** and where to find it. Since 1.1.17 every
  archive carries both emulators, so the choice is no longer between downloads -
  it is between two files inside one. Say which the platform runs by default and
  where the other one lives, because on macOS the interpreter is inside the
  bundle and invisible to anyone who does not already know it is there.
