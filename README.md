...
- **Community Network: one shared network, no server to run.** Tick a box and this machine joins a network shared with other people running RPCEmu, for reaching each other's ShareFS discs and printers. It is public and unencrypted, so it asks you to agree to what that means before it joins. Can be used at the same time as a server of your own. See [docs/community-network.md](docs/community-network.md).
...
| `docs/community-network.md` | The shared network anybody can join, what it is not safe for, and using it alongside a server of your own |
...
An option that takes a value accepts it either as `--option value` or `--option=value`,
except `--fetch-riscos` and `--pkg-list`: both are also complete with no value at all, so
only `=` is accepted for the value they can optionally be given, keeping the two cases
unambiguous.

| Option                           | Effect                                                                                                                                                                                                                                                                                                                                                              |
| -------------------------------- | ------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------- |
| `--machine <name>`               | Run this machine, skipping the selector.                                                                                                                                                                                                                                                                                           |
...
| `--datadir <dir>`                | Where machines, ROMs and settings live, for this run only. Outranks `RPCEMU_DATADIR` and the folder chosen on first run, and is deliberately not remembered, so a scripted run cannot become the default for later interactive ones.                                                                                                |
...
| `--fetch-riscos-name <name>`     | With `--fetch-riscos`, name the machine this instead of deriving a name from the RISC OS version.                                                                                                                                                                                                                       |
