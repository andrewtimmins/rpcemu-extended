...
- **A guest network you can choose** — the machines here sit on a private `/24` behind NAT, `10.10.10.0/24` unless you change it. Worth changing when two installations share a network and their machines would otherwise be given the same addresses — distinct MAC addresses, one IP address, and nothing to say why nothing arrived. See [docs/guest-network.md](docs/guest-network.md).
