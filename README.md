...
- **Display settings that answer one question each** — two menus rather than a row of contradictory switches. *Settings → RISC OS Screen Size* sets how big the RISC OS desktop is: the best mode for this display, a mode that follows the window as you resize it, or a fixed one picked from what the machine's display memory can actually hold. *Settings → Show In Window* sets how that desktop is drawn: actual size, whole multiples only for sharp pixels, or scaled to fit. Full screen is separate, and all three drawing rules still apply inside it. The machine editor offers the same choices under the same names.
...
| `docs/display.md` | Display settings: how big the RISC OS desktop is against how it is drawn, how the monitor EDID is synthesised, and what the old scaling switches became |
...
- Display settings split into screen size and how it is drawn, with whole-multiple and scale-to-fit drawing, and a screen size that can follow the window
