...
- **A guest network you can choose** — the machines here sit on a private `/24` behind NAT, `10.10.10.0/24` by default. It is a setting now, because the machines of two separate installations were given the same addresses the moment they shared a network — distinct MAC addresses, one IP address, and nothing to say why nothing arrived. A new installation picks its own at random; an existing one is left where it is. See [docs/guest-network.md](docs/guest-network.md).
...
| `docs/guest-network.md`          | Which private network the emulated machines are on, why it can be changed, and what a new installation gets                                                                                                                                                                                                                                                     |
...
- Pyromaniac Networking: share one virtual network with other emulators, including RISC OS Pyromaniac, through a JSON tun/tap server that can live on another computer
