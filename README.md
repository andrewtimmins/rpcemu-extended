...
- **Built-in VNC server** — remote desktop access from any VNC client. The server belongs to the emulator rather than to a machine, so it is up before one is running: a client connecting while the machine selector is open is shown the machine list and can choose one, and the connection carries through into the machine it starts. Port and password live in the emulator's own settings (`rpcemu.cfg`), not in each machine's.
