...
get_string(ARMul_State *state, ARMword address, char *buf, size_t bufsize)
{
  ...
  /* ARMul_LoadByte is a macro that drops state (see hostfs.h). */
  NOT_USED(state);
...
put_string(ARMul_State *state, ARMword address, const char *str)
{
  ...
  /* ARMul_StoreByte is a macro that drops state (see hostfs.h). */
  NOT_USED(state);
  ...
  while (len--) {
    ...
    case 32:
      /* Latin-1 hard space. Cast as the UTF-8 fold above does: char is signed
         on most targets, so a bare 160 is an implementation-defined
         conversion. */
